...
def arch( x_size    = 2,
    ...
    ### occupies 16 BPP after the boot  
    ramdisk_vbase        = 0x02000000
        ...
        for y in xrange( y_size ):
            ...
            if ( y < (y_size - 1) ):
                ...
                if ( x==0 ) and ( y==0 ):
                    ...
                    mapping.addIrq( xcu, index = 10, src = tty,
            ...
            if ( x_io!=0 ) and ( y_io!=0 ) and ( x==( x_size-1 ) ) and ( y==( y_size-1 ) ) :
    ...
    if ioc_type != 'RDK':
        mapping.addGlobal( 'seg_ioc', ioc_base, ioc_size,
                           '__W_', vtype = 'PERI', x = x_io, y = y_io,
                           pseg = 'IOC', local = False, big = True )

    mapping.addGlobal( 'seg_tty', tty_base, tty_size, 
                       '__W_', vtype = 'PERI', x = x_io, y = y_io,
                       pseg = 'TTY', local = False, big = True )

    if ( x_io!=0 ) and ( y_io!=0 ) :
        mapping.addGlobal( 'seg_nic', nic_base, nic_size,
                           '__W_', vtype = 'PERI', x = x_size-1, y = y_size-1,
                           pseg = 'NIC', local = False, big = True )

        mapping.addGlobal( 'seg_cma', cma_base, cma_size,
                           '__W_', vtype = 'PERI', x = x_size-1, y = y_size-1,
                           pseg = 'CMA', local = False, big = True )

        mapping.addGlobal( 'seg_fbf', fbf_base, fbf_size,
                           '__W_', vtype = 'PERI', x = x_size-1, y = y_size-1,
                           pseg = 'FBF', local = False, big = True )

        mapping.addGlobal( 'seg_pic', pic_base, pic_size,
                           '__W_', vtype = 'PERI', x = x_size-1, y = y_size-1,
                           pseg = 'PIC', local = False, big = True )
